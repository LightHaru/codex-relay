...
int main(int argc, char **argv) {
    ...
    if (snprintf(executable, sizeof(executable), "%s/ChatGPT", directory) >=
        ...
        fprintf(stderr, "Codex Relay launcher: executable path is too long\n");
        return EXIT_FAILURE;
    }

    const char *home = getenv("HOME");
    if (home == NULL || home[0] == '\0') {
        fprintf(stderr, "Codex Relay launcher: HOME is not set\n");
        return EXIT_FAILURE;
    }
    ...
    if (snprintf(profile, sizeof(profile),
        ...
        fprintf(stderr, "Codex Relay launcher: profile path is too long\n");
        return EXIT_FAILURE;
    }
